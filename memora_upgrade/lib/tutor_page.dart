...
class TutorProfile {
  ...
  factory TutorProfile.fromJson(Map<String, dynamic> json) {
    ...
    return TutorProfile(
      ...
      recommendedModels: rawModels is List
          ? rawModels.map((e) => e.toString()).where((e) => e.trim().isNotEmpty).toList()
          : const [],
...
const List<TutorProfile> _defaultTutors = [
  ...
  TutorProfile(
    ...
    instructions:
        'Actúa como profesor y revisor de programación. Explica qué hace el código y por qué. Ante errores, identifica la causa raíz. Da código completo cuando haga falta y explica las líneas importantes. Prioriza seguridad, mantenibilidad y simplicidad. Compara alternativas y crea ejercicios progresivos.',
    recommendedModels: [
  ...
  TutorProfile(
    ...
    instructions:
        'Actúa como profesor de idiomas. Detecta el idioma objetivo. Adapta el nivel, combina explicación en español con práctica en el idioma objetivo, corrige gramática, vocabulario y naturalidad, usa mini diálogos y permite inmersión cuando se pida.',
    recommendedModels: [
  ...
  TutorProfile(
    ...
    instructions:
        'Usa el método socrático. Haz preguntas útiles antes de entregar una solución completa; si el estudiante está bloqueado, da pistas graduales.',
    recommendedModels: [
  ...
  TutorProfile(
    ...
    instructions:
        'Prioriza velocidad y retención. Da respuestas concisas, usa palabras clave, mini-resúmenes y reglas fáciles de recordar.',
    recommendedModels: [
  ...
  TutorProfile(
    ...
    instructions:
        'Explica cada concepto con ejemplos concretos y analogías cotidianas. Conecta después el ejemplo con la definición formal.',
    recommendedModels: _generalModels,
    isBuiltIn: true,
  ),
];
...
class _TutorPageState extends State<TutorPage> {
  ...
  List<StudyGuide> get activeGuides => widget.store.guides
      .where((guide) => activeTutor.guideIds.contains(guide.id))
      .toList();
  ...
  List<TutorProfile> _upgradeProfiles(List<TutorProfile> loaded) {
    ...
    for (final tutor in loaded) {
      final builtIn = defaultsById[tutor.id];
      upgraded.add(
        tutor.copyWith(
          recommendedModels: tutor.recommendedModels.isEmpty
              ? List<String>.from(builtIn?.recommendedModels ?? _generalModels)
              : tutor.recommendedModels,
          guideIds: tutor.guideIds
              .where((id) => widget.store.guides.any((guide) => guide.id == id))
              .toList(),
        ),
      );
    }
            ...
            child: Column(
              children: [
                const Padding(
                  ...
                  child: Text('Asignar guías a este tutor', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                ),
                Expanded(
                ...
                Padding(
                  ...
                  child: FilledButton(
                    ...
                    child: Text('Guardar ${working.length} guía(s)'),
                  ),
                ),
              ],
            ),
  ...
  Future<void> _ask() async {
    ...
    if (guides.isEmpty) {
      setState(() => answer = 'Asigna al menos una guía a ${activeTutor.name} antes de preguntar.');
      return;
    }
    setState(() => busy = true);
    try {
...
- Usa prioritariamente las bases asignadas que aparecen abajo.
- Si la respuesta no está en ellas, dilo claramente y no inventes datos.
- Si usas varias fuentes, indica de cuál proviene cada punto cuando sea útil.

BASES ASIGNADAS:
${_knowledgeContext(guides)}

PREGUNTA: $question''',
      );
      if (mounted) setState(() => answer = result);
    } catch (e) {
      if (mounted) setState(() => answer = 'No pude consultar ${_sourceLabel(activeTutor.modelSource)}: $e');
    } finally {
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 10),
                TextField(controller: description, minLines: 2, maxLines: 3, decoration: const InputDecoration(labelText: 'Descripción / estilo')),
                const SizedBox(height: 10),
                TextField(controller: instructions, minLines: 3, maxLines: 7, decoration: const InputDecoration(labelText: 'Prompt / instrucciones del tutor')),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  initialValue: source,
                  decoration: const InputDecoration(labelText: 'IA / modelo de este tutor'),
                  items: const [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    ...
                    helperText: 'Lista de referencia; no descarga modelos.',
                  ),
                ),
              ],
            ),
              ...
              onPressed: () {
                ...
                Navigator.pop(
                  ...
                  TutorProfile(
                    ...
                    modelSource: source,
                    recommendedModels: models.text.split('\n').map((e) => e.trim()).where((e) => e.isNotEmpty).toSet().toList(),
  ...
  Future<void> _editActiveTutor() async {
    final edited = await _editTutorDialog(existing: activeTutor);
  ...
  Future<void> _deleteActiveTutor() async {
    ...
    final yes = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('¿Eliminar a ${activeTutor.name}?'),
        actions: [
    ...
    if (yes != true) return;
    setState(() {
      tutors = tutors.where((t) => t.id != activeTutorId).toList();
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    const Expanded(child: Text('Mis tutores', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold))),
                    IconButton(onPressed: () => Navigator.pop(sheetContext, 'add'), icon: const Icon(Icons.person_add_alt_1)),
                  ],
                ),
              ),
              Expanded(
                child: ListView(
                  children: [
                    for (final tutor in tutors)
                      ListTile(
                        leading: Text(tutor.emoji, style: const TextStyle(fontSize: 26)),
                        title: Text(tutor.name),
                        subtitle: Text('${_sourceLabel(tutor.modelSource)} • ${tutor.guideIds.length} guía(s)'),
                        trailing: tutor.id == activeTutorId ? const Icon(Icons.check_circle) : null,
                        onTap: () {
              ...
              Padding(
                ...
                child: Row(
                  children: [
                    Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.pop(sheetContext, 'edit'), icon: const Icon(Icons.edit), label: const Text('Editar activo'))),
                    const SizedBox(width: 8),
                    Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.pop(sheetContext, 'delete'), icon: const Icon(Icons.delete_outline), label: const Text('Eliminar'))),
                  ],
                ),
              ),
            ],
  ...
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(
          ...
          actions: [
            IconButton(onPressed: _openManager, tooltip: 'Administrar tutores', icon: const Icon(Icons.groups_2_outlined)),
            IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const LlmSettingsPage())), icon: const Icon(Icons.tune)),
          ],
        ),
        body: loadingTutors
            ...
            : ListView(
                ...
                children: [
                  DropdownButtonFormField<String>(
                    ...
                    decoration: const InputDecoration(labelText: 'Tutor activo', prefixIcon: Icon(Icons.school_outlined)),
                    items: [for (final tutor in tutors) DropdownMenuItem(value: tutor.id, child: Text('${tutor.emoji} ${tutor.name}', overflow: TextOverflow.ellipsis))],
                    ...
                    child: ExpansionTile(
                      ...
                      children: [
                        for (final model in activeTutor.recommendedModels)
                          Align(alignment: Alignment.centerLeft, child: Padding(padding: const EdgeInsets.only(bottom: 5), child: SelectableText('• $model'))),
                        OutlinedButton.icon(onPressed: _editActiveTutor, icon: const Icon(Icons.tune), label: const Text('Editar modelo / prompt')),
                      ],
                    ),
                  ),
                  const SizedBox(height: 10),
                  Card(
                    child: ExpansionTile(
                      ...
                      title: const Text('Biblioteca de este tutor'),
                      subtitle: Text('${activeGuides.length} guía(s) asignada(s)'),
                      childrenPadding: const EdgeInsets.fromLTRB(16, 0, 16, 14),
                      children: [
                        ...
                        for (final guide in activeGuides)
                          ListTile(dense: true, contentPadding: EdgeInsets.zero, leading: Icon(guide.sourceType == 'xlsx' ? Icons.table_chart : Icons.description_outlined), title: Text(guide.title), subtitle: Text(guide.sourceType.toUpperCase())),
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(children: [Text(activeTutor.emoji, style: const TextStyle(fontSize: 26)), const SizedBox(width: 8), Expanded(child: Text(activeTutor.name, style: const TextStyle(fontWeight: FontWeight.bold)))]),
                          const Divider(),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      suffixIcon: IconButton(onPressed: busy ? null : _ask, icon: busy ? const Padding(padding: EdgeInsets.all(12), child: CircularProgressIndicator()) : const Icon(Icons.send)),
                    ),
                    onSubmitted: (_) => _ask(),
                  ),
                ],
