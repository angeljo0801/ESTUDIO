...
# quick greetings, simple-request fast routing, compact retrieval for short queries,
# visible thinking state, streaming via the shared AiService, and the agent's own
# persistent prompt/model/assigned knowledge bases.
...
new_run = r'''  Future<void> _runAgent() async {
    ...
    try {
      if (isGreeting) {
        ...
        if (mounted) {
          setState(() => answer = spanish
              ? '¡Hola! Soy ${agent.name}. ¿En qué puedo ayudarte?'
      ...
      var context = KnowledgeRetriever.buildContext(
        guides: activeGuides,
        query: '${agent.prompt}\n$request',
      ...
      if (context.startsWith('(No matching evidence found')) {
        context = KnowledgeRetriever.buildCoverageContext(
          guides: activeGuides,
          maxChars: simple && responseMode == 'fast' ? 3600 : limits.$1,
          maxChunks: simple && responseMode == 'fast' ? 4 : limits.$2,
        );
      }

      final answerStyle = responseMode == 'fast'
          ? 'Answer directly and concisely. No preamble or unnecessary repetition.'
          : responseMode == 'deep'
              ? 'Give a thorough but focused answer, following the agent instructions carefully.'
              : 'Give a clear, focused answer with enough context to be useful.';

      final result = await AiService.askConfigured(
        providerOverride: agent.modelSource == 'global' ? 'global' : agent.modelSource,
        responseMode: simple && responseMode == 'fast' ? 'fast' : responseMode,
        onPartial: (partial) {
          if (mounted && partial.isNotEmpty) setState(() => answer = partial);
        },
        prompt: '''You are ${agent.name}, a custom Memora agent.
