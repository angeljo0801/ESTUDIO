...
object LocalAiManagerBridge {
            ...
            when (call.method) {
                "ask" -> {
                    ...
                    val data = Bundle().apply {
                        ...
                        putInt("maxTokens", call.argument<Int>("maxTokens") ?: 320)
                        putDouble("temperature", call.argument<Double>("temperature") ?: 0.2)
                    }
    ...
    private fun ensureBound() {
        ...
        try {
            ...
            binding = false
            failAll("No pude abrir Local AI Manager: ${e.message ?: "error de conexión"}")
        }
    }
        ...
        while (queue.isNotEmpty()) {
            ...
            try {
                ...
                pending.remove(item.id)?.error(
                    "LOCAL_AI_MANAGER",
                    "No pude enviar la solicitud al Manager: ${e.message ?: "conexión cerrada"}",
                    null
                )
                ensureBound()
                return
