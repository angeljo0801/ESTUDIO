...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine:FlutterEngine){
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger,"com.memora/daily_exam_alarm").setMethodCallHandler{call,result->
            ...
            when(call.method){"schedule"->{val h=call.argument<Int>("hour")?:6;val m=call.argument<Int>("minute")?:0;prefs.edit().putBoolean("flutter.daily_exam_auto_enabled",true).putLong("flutter.daily_exam_auto_hour",h.toLong()).putLong("flutter.daily_exam_auto_minute",m.toLong()).apply();DailyExamAlarm.schedule(appContext,h,m);result.success(null)};"cancel"->{prefs.edit().putBoolean("flutter.daily_exam_auto_enabled",false).apply();DailyExamAlarm.cancel(appContext);result.success(null)};else->result.notImplemented()}}
...
object DailyExamAlarm {
    ...
    fun schedule(context:Context,hour:Int,minute:Int){val alarm=context.getSystemService(Context.ALARM_SERVICE) as AlarmManager;val pi=PendingIntent.getBroadcast(context,REQUEST,Intent(context,DailyExamAlarmReceiver::class.java),PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE);val c=Calendar.getInstance().apply{set(Calendar.HOUR_OF_DAY,hour);set(Calendar.MINUTE,minute);set(Calendar.SECOND,0);set(Calendar.MILLISECOND,0);if(timeInMillis<=System.currentTimeMillis())add(Calendar.DAY_OF_YEAR,1)};if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.M)alarm.setExactAndAllowWhileIdle(AlarmManager.RTC_WAKEUP,c.timeInMillis,pi)else alarm.setExact(AlarmManager.RTC_WAKEUP,c.timeInMillis,pi)}
...
class DailyExamAlarmReceiver:BroadcastReceiver(){override fun onReceive(context:Context,intent:Intent?){val p=context.getSharedPreferences("FlutterSharedPreferences",Context.MODE_PRIVATE);if(!p.getBoolean("flutter.daily_exam_auto_enabled",false))return;DailyExamAlarm.schedule(context,p.getLong("flutter.daily_exam_auto_hour",6).toInt(),p.getLong("flutter.daily_exam_auto_minute",0).toInt());val launch=context.packageManager.getLaunchIntentForPackage(context.packageName);val pi=launch?.let{PendingIntent.getActivity(context,4611,it,PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)};val nm=context.getSystemService(NotificationManager::class.java);if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O)nm.createNotificationChannel(NotificationChannel("memora_daily_exam","Memora Daily Exams",NotificationManager.IMPORTANCE_DEFAULT));val b=Notification.Builder(context,"memora_daily_exam").setSmallIcon(R.drawable.ic_stat_memora).setContentTitle("Daily Exams scheduled").setContentText("Memora is ready to generate today's tutor exams.").setAutoCancel(true);if(pi!=null)b.setContentIntent(pi);nm.notify(4610,b.build())}}
