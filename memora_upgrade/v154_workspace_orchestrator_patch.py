...
# Remove the matching Crear destination without touching the creator itself.
...
# Add a floating creator button to the existing Library Scaffold.
...
p=Path('lib/general_ai_chat_page.dart'); g=p.read_text()
for imp in ["import 'chat_store.dart';\n", "import 'llm_settings_page.dart';\n"]:
    if imp not in g: g=g.replace("import 'ai_service.dart';\n", "import 'ai_service.dart';\n"+imp, 1)
# Replace the simple in-memory implementation with a full session implementation.
...
# Add Orchestrator entry + AI settings to Agents without replacing existing agent features.
...
# Add toolbar actions before existing create-agent button.
