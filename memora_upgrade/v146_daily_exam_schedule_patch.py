...
load="""    autoDailyEnabled = prefs.getBool('daily_exam_auto_enabled') ?? false;
    ...
    autoDailyPending = prefs.getBool('daily_exam_auto_pending') ?? false;
"""
...
helpers=r'''  String _clock(TimeOfDay t) { final h=t.hourOfPeriod==0?12:t.hourOfPeriod; return '$h:${t.minute.toString().padLeft(2,'0')} ${t.period==DayPeriod.am?'AM':'PM'}'; }
  ...
  Future<void> _runAutomaticDailyIfDue() async { if(!autoDailyEnabled||autoDailyRunning)return; final prefs=await SharedPreferences.getInstance(); autoDailyPending=prefs.getBool('daily_exam_auto_pending')??autoDailyPending; final now=DateTime.now(); final scheduled=DateTime(now.year,now.month,now.day,autoDailyTime.hour,autoDailyTime.minute); if(!autoDailyPending&&(now.isBefore(scheduled)||autoDailyLastRun==today))return; autoDailyRunning=true; await prefs.setBool('daily_exam_auto_pending',true); autoDailyPending=true; try { for(final tutor in tutors){if(_dailyFor(tutor.id)!=null||TutorContextService.guidesFor(widget.store,tutor).isEmpty)continue;try{final exam=await _buildExam(tutor:tutor,count:10,difficulty:'Adaptativa',generatorMode:dailyGenerator,daily:true);final copy=List<TutorExamSession>.from(sessions)..removeWhere((e)=>e.isDaily&&e.tutorId==tutor.id&&e.dateKey==today)..insert(0,exam);sessions=copy;await _saveSessions();await prefs.setString('daily_exam_last_${tutor.id}',today);}catch(_){}} final missing=tutors.any((t)=>TutorContextService.guidesFor(widget.store,t).isNotEmpty&&_dailyFor(t.id)==null);if(!missing){autoDailyLastRun=today;autoDailyPending=false;await prefs.setString('daily_exam_auto_last_run',today);await prefs.setBool('daily_exam_auto_pending',false);}if(mounted)setState((){});}finally{autoDailyRunning=false;} }
